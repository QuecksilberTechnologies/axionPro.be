...
namespace axionpro.api.Controllers.Employee
{
    ...
    public class DependentController : ControllerBase
    {
        ...
        public async Task<IActionResult> Update([FromBody] GenricUpdateRequestDTO dto)
        {
            try
            {
                _logger.LogInfo($"Updating employee-Dependent record. EmployeeId: {dto.EmployeeId}");
                ...
                if (!result.IsSucceeded)
                {
                    _logger.LogInfo($"Failed to update employee-Dependent with Id: {dto.EmployeeId}");
                    return BadRequest(result);
                }
