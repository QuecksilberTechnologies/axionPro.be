...
namespace axionpro.api.Controllers.Insurance
{
    ...
    public class PolicyTypeInsuranceMapController : ControllerBase
    {
        ...
        /// <para>Response DTO property analysis: ApiResponse: IsSucceeded (bool), Message (string), Data (T), Errors (List&lt;string&gt;), ErrorCode (string?), PageNumber (int?), PageSize (int?), TotalRecords (int?), TotalPages (int?), IsPrimaryMarked (bool?), HasAllDocUploaded (bool?), CompletionPercentage (double?); GetPolicyTypeInsuranceMapDetailsResponseDTO: Id (int), PolicyTypeId (int), InsuranceTypeId (int), MaxChildAllowed (int), MaxSpouseAllowed (int), PolicyName (string), InsuranceTypeName (string), FileName (string), FilePath (string), Description (string?), IsActive (bool), ParentsAllowed (bool), InLawsAllowed (bool)</para>
