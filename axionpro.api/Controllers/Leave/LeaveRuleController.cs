...
namespace axionpro.api.Controllers.Leave
{
    ...
    public class LeaveRuleController : ControllerBase
    {
        ...
        /// <remarks>
        /// <para>Angular usage status: Used-In-Angular.</para>
        /// <para>API endpoint purpose: retrieves all leave rule.</para>
        /// <para>Handler flow: GetAllLeaveRuleQuery is processed by GetAllLeaveRuleQueryHandler; operation(s): GetLeaveRuleAsync.</para>
        /// <para>Response DTO property analysis: ApiResponse: IsSucceeded (bool), Message (string), Data (T), Errors (List&lt;string&gt;), ErrorCode (string?), PageNumber (int?), PageSize (int?), TotalRecords (int?), TotalPages (int?), IsPrimaryMarked (bool?), HasAllDocUploaded (bool?), CompletionPercentage (double?); GetLeaveRuleResponseDTO: Id (long), TenantId (long), PolicyLeaveTypeId (long), ApplySandwichRule (bool), IsHalfDayAllowed (bool), LeaveTypeId (int?), LeaveName (string?), HalfDayNoticeHours (int?), NoticePeriodDays (int?), MaxContinuousLeaves (int?), MinGapBetweenLeaves (int?), IsActive (bool), IsSoftDeleted (bool), IsLinkedSandwichRule (bool?), Remark (string?), AddedById (long), AddedDateTime (DateTime), UpdatedById (long?), UpdatedDateTime (DateTime?)</para>
        ...
        /// <remarks>
        /// <para>Angular usage status: Used-In-Angular.</para>
        /// <para>API endpoint purpose: updates leave rule.</para>
        /// <para>Handler flow: UpdateLeaveRuleCommand is processed by UpdateLeaveRuleCommandHandler; operation(s): UpdateLeaveRuleAsync.</para>
        /// <para>Response DTO property analysis: ApiResponse: IsSucceeded (bool), Message (string), Data (T), Errors (List&lt;string&gt;), ErrorCode (string?), PageNumber (int?), PageSize (int?), TotalRecords (int?), TotalPages (int?), IsPrimaryMarked (bool?), HasAllDocUploaded (bool?), CompletionPercentage (double?); GetLeaveRuleResponseDTO: Id (long), TenantId (long), PolicyLeaveTypeId (long), ApplySandwichRule (bool), IsHalfDayAllowed (bool), LeaveTypeId (int?), LeaveName (string?), HalfDayNoticeHours (int?), NoticePeriodDays (int?), MaxContinuousLeaves (int?), MinGapBetweenLeaves (int?), IsActive (bool), IsSoftDeleted (bool), IsLinkedSandwichRule (bool?), Remark (string?), AddedById (long), AddedDateTime (DateTime), UpdatedById (long?), UpdatedDateTime (DateTime?)</para>
        ...
        /// <remarks>
        /// <para>Angular usage status: Used-In-Angular.</para>
        /// <para>API endpoint purpose: deletes leave rule.</para>
        /// <para>Handler flow: DeleteLeaveRuleCommand is processed by DeleteLeaveRuleCommandHandler; operation(s): GetLeaveRuleByIdAsync, DeleteLeaveRuleAsync.</para>
