...
namespace axionpro.application.Common.Helpers
{
    public static class EmployeeBankInfoMapperHelper
    {
        ...
        public static GetBankAccessResponseDTO ConvertToAccessResponseDTO<T>(T source)
        {
            var result = new GetBankAccessResponseDTO();

            var sourceProps = typeof(T).GetProperties();
            var targetProps = typeof(GetBankAccessResponseDTO).GetProperties();
