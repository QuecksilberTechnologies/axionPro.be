...
namespace axionpro.application.Common.Helpers
{
    ...
    {
        public static GetBaseAccessEmployeeResponseDTO ConvertToAccessResponseDTO<T>(T source)
        {
            var result = new GetBaseAccessEmployeeResponseDTO();

            var sourceProps = typeof(T).GetProperties();
            var targetProps = typeof(GetBaseAccessEmployeeResponseDTO).GetProperties();
