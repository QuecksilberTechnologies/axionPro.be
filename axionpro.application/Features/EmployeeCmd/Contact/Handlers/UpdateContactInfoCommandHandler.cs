    ...
    { 
        ...
        public async Task<ApiResponse<bool>> Handle(UpdateContactInfoCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    _logger.LogInformation("No Contact record found for Id: {Id}", dto.EncriptedId);
                ...
                ContactEntity.UpdatedById = dto.EmployeeId;
                ...
                if (!updateStatus)
                {
                    _logger.LogError("Failed to update EmployeeContactDetail for Id: {Id}", dto.EncriptedId);
                    return ApiResponse<bool>.Fail("Failed to update employee Contact record.");
                }

                _logger.LogInformation("Field '{Field}' updated successfully for ContactId: {Id}", dto.FieldName, dto.EncriptedId);
