...
namespace axionpro.application.Features.EmployeeCmd.EmployeeBase.Handlers
{
    ...
    public class UpdateBaseEmployeeOfficialCommand : IRequest<ApiResponse<bool>>
    {
        public UpdateEmployeeRequestOfficialDTO DTO { get; }

        public UpdateBaseEmployeeOfficialCommand(UpdateEmployeeRequestOfficialDTO dto)
        {
            DTO = dto;
        }
    }
    ...
    public class UpdateBaseEmployeeOfficialCommandHandler : IRequestHandler<UpdateBaseEmployeeOfficialCommand, ApiResponse<bool>>
    {
        private readonly IBaseEmployeeRepository _employeeRepository;
        private readonly IUnitOfWork _unitOfWork;
        private readonly ILogger<UpdateBaseEmployeeOfficialCommandHandler> _logger;
        ...
        public UpdateBaseEmployeeOfficialCommandHandler(
            IBaseEmployeeRepository employeeRepository,
            IUnitOfWork unitOfWork,
            ILogger<UpdateBaseEmployeeOfficialCommandHandler> logger,
            IMapper mapper,
        ...
        public async Task<ApiResponse<bool>> Handle(UpdateBaseEmployeeOfficialCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (request.DTO.DepartmentId.HasValue)
                    existingEmployee.DepartmentId = request.DTO.DepartmentId.Value;

                if (request.DTO.EmployeeTypeId.HasValue)
                    existingEmployee.EmployeeTypeId = request.DTO.EmployeeTypeId.Value;
    ...
    // ---------- ROLE MANAGEMENT (PRIMARY + SECONDARY) ----------

    //var hasPrimary = request.DTO.PrimaryRoleId.HasValue;
    //var hasSecondary = request.DTO.SecondryRoleIds != null && request.DTO.SecondryRoleIds.Any();

    //if (hasPrimary || hasSecondary)
    //{
    //    // 🔐 Permission check
    //    if (!permissions.Contains("UpdateUserRoles"))
    //    {
    //        await _unitOfWork.RollbackTransactionAsync();
    //        return ApiResponse<bool>.Fail("You are not allowed to update roles.");
    //    }

    //    // Remove existing roles ONLY when role update is requested
    //    existingEmployee.UserRoles.Clear();

    //    // ---------- PRIMARY ROLE ----------
    //    if (hasPrimary)
    //    {
    //        existingEmployee.UserRoles.Add(new UserRole
    //        {
    //            EmployeeId = existingEmployee.Id,
    //            RoleId = request.DTO.PrimaryRoleId!.Value,
    //            IsPrimaryRole = true,
    //            IsActive = true,
    //            RoleStartDate = DateTime.UtcNow,
    //            UpdatedById = request.DTO.Prop.UserEmployeeId,
    ...
    //    if (hasSecondary)
    //    {
    //        foreach (var roleId in request.DTO.SecondryRoleIds!.Distinct())
    //        {
    //            if (hasPrimary && roleId == request.DTO.PrimaryRoleId)
    //                continue;
