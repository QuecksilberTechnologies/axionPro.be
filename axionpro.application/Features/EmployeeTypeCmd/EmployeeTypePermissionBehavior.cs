...
// Role    : CEO
// Purpose : Centrally enforces EmployeeType module-operation permissions.
// ================================================================
...
namespace axionpro.application.Features.EmployeeTypeCmd;

/// <summary>
/// Enforces the current tenant user's database-backed EmployeeType permissions
/// before any EmployeeType command or query reaches its handler.
/// </summary>
...
public sealed class EmployeeTypePermissionBehavior<TRequest, TResponse>(
    IUnitOfWork unitOfWork,
    ICommonRequestService commonRequestService,
    ILogger<EmployeeTypePermissionBehavior<TRequest, TResponse>> logger)
    : IPipelineBehavior<TRequest, TResponse>
    where TRequest : notnull
{
    private const string EmployeeTypeModuleCode = BulkImportConstants.EmployeeTypeModuleCode;
        ...
        CancellationToken cancellationToken)
    {
        if (typeof(TRequest).Namespace?.StartsWith(
                "axionpro.application.Features.EmployeeTypeCmd",
        ...
        if (!string.Equals(moduleCode, EmployeeTypeModuleCode, StringComparison.OrdinalIgnoreCase))
        {
            logger.LogWarning(
                "EmployeeType module-code mismatch for {EmployeeTypeRequest}. ModuleId: {ModuleId}, ModuleCode: {ModuleCode}, ExpectedModuleCode: {ExpectedModuleCode}",
                typeof(TRequest).Name,
                permissionRequest.ModuleId,
                moduleCode,
                EmployeeTypeModuleCode);
        ...
        {
            logger.LogWarning(
                "Invalid Tenant authorization context for EmployeeType request {EmployeeTypeRequest}. TenantId: {TenantId}, EmployeeId: {EmployeeId}, TokenRoleId: {TokenRoleId}",
                typeof(TRequest).Name,
                validation.TenantId,
        ...
        // assignments, so a valid but stale JWT cannot grant EmployeeType access.
