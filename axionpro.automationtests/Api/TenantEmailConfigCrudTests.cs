...
public sealed class TenantEmailConfigCrudTests
{
    ...
    public async Task AuthenticateAndResolveTenantEmailConfigPermissions()
    {
        ...
        Assert.That(
            ...
            Is.True,
            $"The test account's tenant does not expose the '{TenantEmailConfigModuleCode}' module. Apply database-scripts/complete seed data/AxionPro_New_Production_Module_Operation_Seed.sql, enable the module and its CRUD operations for the tenant through plan-entitlement synchronization, and assign the operations to the account's Tenant role.");
