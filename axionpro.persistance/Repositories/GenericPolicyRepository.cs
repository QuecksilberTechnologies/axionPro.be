...
public sealed class GenericPolicyRepository(WorkforceDbContext context) : IGenericPolicyRepository
{
    ...
    {
        var employee = await context.Employees.AsNoTracking().FirstOrDefaultAsync(x => x.Id == dto.EmployeeId && x.TenantId == tenantId, cancellationToken) ?? throw new NotFoundException("Employee was not found.");
