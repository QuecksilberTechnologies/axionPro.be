...
BEGIN
    ...
    END LOOP;
    IF (SELECT count(*) FROM axionpro."Module" WHERE "ModuleCode"='DEPARTMENT' AND "ModuleScope"=1) <> 1 THEN
        RAISE EXCEPTION 'Exactly one DEPARTMENT baseline module is required';
    END IF;
END $seed$;
...
FROM axionpro."Module" source WHERE source."ModuleCode"='DEPARTMENT'
...
JOIN axionpro."Module" baseline ON baseline."Id"=source."ModuleId" AND baseline."ModuleCode"='DEPARTMENT'
